%% ToDo:
%% • Gute schwarz-weiße vektorisierte Grafiken finden – oder lieber mit TikZ selbst erstellen?
%% • Den Teil über die Gebrochenen Schriften in einer setzen?
%% • …
%% 
%% Zu den Bildern:
%% 
%% Ist es ratsam, so wie bisher sechs verschiedene Bilder einzubinden? Dann:
%% • Da die aber zu klein sind, sollte man dann immer nur eine Ebene pro Bild machen.
%% • Es sollten unbedingt alle Ebenen mit rein, also Ebenen 2 bis 6 (so, wie es auch jetzt ist),
%% • und diese so wie Abbildung 1 (Ebene 2), nur auf der gesamten Breite.
%% 
%% Alternativ:
%% Da die DTK im A5-Format gedruckt wird, könnte man stattdessen auch die Druckvorlage (also dann
%% mit allen sechs Ebenen) nehmen und sie um 90° rotiert auf eine eigene  Seite packen … das wäre
%% dann quasi fast schon Netbook-Tastaturgröße.
%% • Ist aber unschön, da man so das Heft drehen muss.
%% 
%% Vielleicht auch Ansichtssache.
%% 
%% In beiden Fällen bleibt noch offen, was mit dem Zahlenblock geschenen soll:
%% • Aus Platzgründen auf den Zahlenblock verzichten?
%% • Oder eine separte ›alle sechs Ebenen des Ziffenblocks in einem Bild›-Grafik erstellen?
%% 
%% Das letzte Wort liegt bei allen offen Fragen bei Arno.
%% 
...
\address{Arno}{Trautmann}{Boxbergring 10\\ 69126 HD-Boxberg\\ \protect\url{arno.trautmann@gmx.de}}
\maketitle
\begin{abstract}
...
In der DTK 3/2008 hat Markus Kohm berichtet, dass das große scharfe S normiert wurde. Leider gibt es nur sehr wenige Schriften, in denen diese Glyphe vorhanden ist, aber schon lange vor dieser Meldung kann es mit Neo es direkt eingegeben werden.
